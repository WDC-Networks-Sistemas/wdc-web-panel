...
import KanbanColumnPagination from '@/components/KanbanColumnPagination';
import RejectOrderModal from '@/components/RejectOrderModal';

  const ITEMS_PER_COLUMN = 3;

  const KanbanBoard: React.FC = () => {
  const { orders, updateOrderStatus, updateOrderStatusWithReason, setSelectedOrder, filteredOrders, resetFilters } = useOrders();
  ...
  const approvedOrders = filteredOrders('approved');
  const rejectedOrders = filteredOrders('rejected');
